...
> | **State split** | 10 open · 3 closed · 3 merged *(completely unchanged vs the 2026-07-27 08:51 UTC baseline — no membership, state, milestone, title, or activity drift; second consecutive fully static window)* |
...
|---|---:|---|---|
| [`WordPress/php-ai-client`](https://github.com/WordPress/php-ai-client) | **24** *(+3)* | **1.4.0** (2026-07-15) | Uniform provider-agnostic PHP client beneath WordPress AI features; changes to models, modalities, schemas, streaming, embeddings, and provider selection can alter the plugin's implementation path. |
| [`WordPress/mcp-adapter`](https://github.com/WordPress/mcp-adapter) | **20** *(+4)* | **v0.5.0** (2026-04-15) | Abilities-to-MCP bridge; transport, exposure, approval, session, schema, and server-registration changes can alter external-agent integrations. |

**Movement in this window (2026-07-27 → 2026-08-01).** Both upstream repositories gained again, and for the second consecutive window neither cut a release. The adapter's backlog is now growing faster than its release cadence: **20 open PRs against a release stream last cut in April**.

- **`php-ai-client` +3 → 24**, nothing merged or closed. New: **#266** "GH#265: Preserve Guzzle HTTP error responses" (superdav42), **#267** "Fix avoid `file_exists()` on oversized strings in File detection" (the-hercules), and — much the largest — **#269** "Add text extraction (OCR / document parsing) capability" (saarnilauri, +1,618 lines). #269 is the one to watch downstream: a new client-level *modality* is exactly the kind of upstream capability that has previously arrived in the plugin ahead of its core route (see embeddings → `WordPress/ai#892` below). No readiness changes this window.
- **`mcp-adapter` +4 net → 20** (six opened, two merged). **Merged 2026-07-27: #251** (prevent concurrent MCP session overwrites) and **#254** (inherit public ability exposure for MCP) — the latter resolves the item flagged last window against the plugin-side ability-governance thread (`WordPress/ai#863`/#354); MCP exposure now inherits from public ability exposure upstream, while the plugin-side toggle design (#863, PR #881) is still unsettled. **Opened:** #258 "Feat/resource meta" (AshishJii), #259 "Nullable singletons, WP-CLI alias & notices" (murshed), and a four-PR DTO-normalization cluster from galatanovidiu, all four targeting the same upstream issue **mcp-adapter#245**: comprehensive draft **#260** (+3,407, "Normalize `_meta` and annotations on every emitted DTO, and emit `mimeType` as declared", *Closes #245*) and three narrower same-scope cuts opened the next day — **#262** (emit `mimeType` as declared), **#263** (carry `_meta` through to every emitted DTO, also *Closes #245*), and **#264** (keep a malformed annotation from costing the payload, additionally *Closes #261*). Because #260 and #263 both declare `Closes #245`, the four overlap rather than partition; counting them separately inflates the +4 — treat it as one workstream. Readiness change: **#244** (filterable MCP exposure check for the built-in discover/get-info/execute tools) went BLOCKED → DIRTY.
- **Standing cross-repo consequence, unchanged:** php-ai-client's shipped 1.4.0 embeddings support plus the WP 7.1 → 7.2 slip of `wordpress-develop#12530` is still why `WordPress/ai` PR **#892** vendors that embedding code into the plugin behind an `SDK_Overlay`. That PR remains open and uncarded.
