...
> | | |
> |---|---|
> | **Data snapshot** | 2026-07-03 (latest item activity: 2026-07-03) |
> | **Items captured** | 265 (full board, via GraphQL Projects API) |
...
> | **In active development** | **v1.2.0** (32 open / 0 Done; milestone due **2026-07-30**) — the sole dated build wave now that v1.1.0 has shipped; the uncommitted roadmap backlog sits in **Future Release** (40) |
...
> 2. **[`wordpress-ai-open-issues.md`](./wordpress-ai-open-issues.md)** — deep per-issue dossiers (problem · approach · decisions · dependencies · discussion), grouped by status. *(Current to 2026-07-03 — all 56 current board-open issues dossiered, plus #84 retained as a removed-board reference and 16 recently board-Done issues retained for reference.)*
> 3. **[`wordpress-ai-planned-work.md`](./wordpress-ai-planned-work.md)** — the release-ordered delivery plan for all 76 non-Done board cards, including 19 open board-tracked PRs and stale merged PR #484.
...
| Dimension | Breakdown |
|---|---|
| **Total items** | **265** = 194 PRs + 71 issues |
| **By status** | Done **189** · In progress **29** · In discussion/Needs decision **21** · Needs review **10** · Backlog **7** · To do **8** · Triage **1** |
| **Open work** | **76 non-Done board items** = 56 open issues + 20 non-Done PR cards *(19 open; #484 is merged but still board-Needs review)* |
| **By repo** | `WordPress/ai` **264** · `WordPress/ai-provider-for-google` **1** (#23). `WordPress/abilities-api` #84 is no longer on Project #240. |
...
**Caveat for maintainers:** "Done" (189) is overwhelmingly merged PRs; the *forward-looking roadmap signal lives in the 56 open issues*, not the PRs.

---

## 2. Release cadence

A very fast train of **17 fully-shipped releases**, with **1.1.0 the current public version** (shipped 2026-07-01). **1.2.0 is now the sole active build wave** (32 open); the uncommitted roadmap backlog sits in Future Release (40).
...
| **1.1.0** | 22 | ✅ Done | **Latest shipped (2026-07-01)** — encryption PR #560 merged (2026-06-30) and release issue #805 closed board-Done; 27 → 22 this refresh as five already-Done bug issues (#390/#391/#571/#578/#678) were de-carded from the board 2026-07-03 |
| **1.2.0** | 32 | 📋 all open | **The sole active build wave** — absorbed the bulk of the former 1.1.0 lane on 2026-06-30; unchanged this refresh (0 Done / 32 open) |
| **Future Release** | 40 | 📋 40 open (33 issues + 7 PRs) | The real roadmap backlog; unchanged this refresh |
| _(no milestone)_ | 23 | 20 Done / 3 open | The 3 open are #809 (Content-Summary nested-block detection) + #816 (Type-Ahead front-end/WooCommerce break, now In progress) + board-new #818 (missing alt text on AI Home feature card) |

*Sums to 265 (134 + 3 + 8 + 3 + 22 + 32 + 40 + 23).*
...
Board-Done now sits at 189 items (196 → 189 as seven already-Done bug/release issues were de-carded from the board 2026-07-03 — #390/#391/#571/#578/#678 from the 1.1.0 milestone and #589/#727 from 1.0.2; all shipped, just no longer board cards). The public shipped line is now the 17 releases through **1.1.0** (shipped 2026-07-01); the 1.1.0-targeted issues/PRs that moved Done through June are now in a public build, and the 1.2.0 lane is the next wave of board-Done work not yet publicly released:
...
- **Board-new / moved this refresh (2026-07-03):** a **missing-alt-text a11y bug** on the AI Home feature card (#818, In progress / no milestone, PR #819 — the `<img>` in `routes/ai-home/stage.tsx` renders with an empty `alt`, failing WCAG 1.1.1); and the **Type-Ahead front-end regression** (#816) moved **Triage → In progress** with fix PR #820. Prior board-new (2026-07-02): Content-Summary **nested-block detection** (#809, In progress / no milestone, PR #810 by Intenzi — the top-level-only block lookup misses summaries moved inside Group/Column) and a **Connector-Approvals access-notice gap** (#815, To do / 1.2.0 — no admin notice tells you the AI plugin still needs provider access until a generation fails; from a WPORG support report, related to #660).
...
*(Full per-issue detail in [§9](#9-appendix--full-open-issue-tracker-56).)*
...
- **#632** *(now board-Done / closed)* deactivate a connector without losing its key · **#660** *(1.2.0)* clearer "blocked by Connector Approvals" error · **#815** *(1.2.0, board-new)* surface an admin notice when Connector Approvals still needs the AI plugin granted access to a connected provider (WPORG support report; related to #660).
...
- **1 Triage:** Core Abilities umbrella (#40) is the sole remaining Triage item — board-new **#816** (the Type Ahead front-end regression) moved **Triage → In progress** on 2026-07-03 with fix PR #820.
- **Blocked:** #425 (alt-text placement).
- **Unmilestoned board work (3):** three cards carry no milestone this refresh — **#809** (Content-Summary nested-block detection, In progress, PR #810), **#816** (Type-Ahead front-end regression, now In progress, PR #820), and board-new **#818** (missing alt text on the AI Home feature card, In progress, PR #819). #643 and #689 carry **Future Release**; everything else committed sits in 1.2.0. The only non-Done non-Future straggler under a shipped milestone is stale merged PR #484 under 0.9.0.
...
3. **Backlog-to-commitment gap** — 40 "Future Release" + 21 "In discussion" items, but the near-term picture is firm: **1.2.0 now carries 32 committed items** (the sole active build wave) and **v1.1.0 shipped on 2026-07-01** (27 Done / 0 open). Still lots of vision in Future Release, but near-term scheduling is no longer thin.
...
5. **Live regressions against shipped features** — #643 (blank Connectors/settings pages on WP 7.0) survived into 1.0.2 but likely points to broken core JS in the reporter's environment; #741 reports a black/blank flicker on AI/Connectors admin pages in AI 1.0.2 and now has a likely `#wpwrap` critical-style root cause plus Gutenberg cross-reference. **#816** is a front-end regression in the Type Ahead experiment that *just shipped in 1.1.0* — it enqueues `wp-editor` on public pages and can intermittently corrupt WooCommerce block checkout; it moved **Triage → In progress** on 2026-07-03 with fix PR #820 (still unmilestoned), being fast-tracked given it lands in a public release.

---

## 9. Appendix — full open-issue tracker (56)

> 📄 **Deep per-issue dossiers** (problem · approach · open decisions · dependencies · discussion) for the open issues live in the companion file **[`wordpress-ai-open-issues.md`](./wordpress-ai-open-issues.md)** *(current to 2026-07-03: all 56 current board-open issues + #84 removed-board reference + 16 recently board-Done issues retained for reference)*. The index table below is updated to the current **56**.
...
| [#809](https://github.com/WordPress/ai/issues/809) | — | Content | Intenzi | Content Summarization: detect the summary group inside nested blocks (Group/Column), not just top level (PR #810) — board-new |
| [#816](https://github.com/WordPress/ai/issues/816) | — | Bug | xuanji86 | Type Ahead enqueues `wp-editor` on the front end → intermittently breaks WooCommerce block checkout (fix PR #820) — moved Triage → In progress |
| [#818](https://github.com/WordPress/ai/issues/818) | — | Bug | — | Missing alt text on the AI Home feature-card `<img>` (empty `alt`; WCAG 1.1.1) — board-new, PR #819 |
...
| [#793](https://github.com/WordPress/ai/issues/793) | 1.2.0 | Infra | — | Developer Tool "Customize experiments": gate advanced experiment settings behind an opt-in toggle |
| [#815](https://github.com/WordPress/ai/issues/815) | 1.2.0 | Bug/Providers | jeffpaul | Connector Approvals: no admin notice that the AI plugin still needs provider access until a generation fails (WPORG report; related #660) — board-new |
...
- "WordPress AI" board == the WordPress/ai Showcase Plugin in practice (264/265 items).
- Roadmap signal = open ISSUES, not PRs (PRs are mostly the 189 "Done").
